        ...
        if error != nil {
            ...
            Auth.auth().createUser(withEmail: email, password: password) { [self] (result, err) in
                ...
                if err != nil {
                    ...
                    db.collection("users").document(result!.user.uid).setData(["firstname": firstName, "lastname": lastName, "high": 0, "uid": result!.user.uid, "time": time, "gems": 0, "speed": 0 ]) { (error) in
                        if error != nil {
